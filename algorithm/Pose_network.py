...
class Pose_network(Network):
    ...
    def build(self):
        ...
        with tf.variable_scope('pose'):
            frame0_ph = tf.placeholder(dtype = tf.float32, shape=(batch_size, img_size[0], img_size[1], 3))
            frame1_ph = tf.placeholder(dtype = tf.float32, shape=(batch_size, img_size[0], img_size[1], 3))
            depth0_ph = tf.placeholder(dtype = tf.float32, shape=(batch_size, img_size[0], img_size[1]))
            depth1_ph = tf.placeholder(dtype = tf.float32, shape=(batch_size, img_size[0], img_size[1]))
            mask0_ph = tf.placeholder(dtype = tf.float32, shape=(batch_size, img_size[0], img_size[1], mask_ch))
            mask1_ph = tf.placeholder(dtype = tf.float32, shape=(batch_size, img_size[0], img_size[1], mask_ch))
            nan0_ph = tf.placeholder(dtype = tf.float32, shape = (batch_size, img_size[0], img_size[1]))
            nan1_ph = tf.placeholder(dtype = tf.float32, shape = (batch_size, img_size[0], img_size[1]))
            bbox0_ph = tf.placeholder(dtype = tf.float32, shape = (batch_size, mask_ch, 6))
            bbox1_ph = tf.placeholder(dtype = tf.float32, shape = (batch_size, mask_ch, 6))
            g_vr0_ph = tf.placeholder(dtype = tf.float32, shape = (batch_size, 4, 4))
            g_vr1_ph = tf.placeholder(dtype = tf.float32, shape = (batch_size, 4, 4))
